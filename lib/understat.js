...
export function buildUnderstatStats(datesData, normalizeTeamName) {
  ...
  for (const m of Object.values(datesData || {})) {
    // SALTA partite future
    if (m?.isResult !== true) continue;

    const homeName = m?.h?.title;
    const awayName = m?.a?.title;
    if (!homeName || !awayName) continue;

    // SALTA se i gol non sono numeri validi (stringa vuota, null)
    const homeGoalsRaw = m?.goals?.h;
    const awayGoalsRaw = m?.goals?.a;
    if (homeGoalsRaw === "" || awayGoalsRaw === "") continue;
    if (homeGoalsRaw == null || awayGoalsRaw == null) continue;

    const homeGoals = Number(homeGoalsRaw);
    const awayGoals = Number(awayGoalsRaw);
    if (!Number.isFinite(homeGoals) || !Number.isFinite(awayGoals)) continue;

    const homeXG = m?.xG?.h === "" || m?.xG?.h == null ? null : Number(m.xG.h);
    const awayXG = m?.xG?.a === "" || m?.xG?.a == null ? null : Number(m.xG.a);
